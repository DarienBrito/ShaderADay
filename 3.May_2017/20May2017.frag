...
Adding a nice function to replicate by IQ
need to wrap my head around that one better! 
Also check the lightning, there are artifacts... how can they be solved?
...
#define TYPE 1 // Type of boolean operation for constructive geometries
...
{
    vec3 q = mod(p,c)-0.5*c; // <--- magic here, I'm just using a sphere to test it!
    return sphere( q, 0.5 ); //
}
...
float rayMarch(vec3 origin, vec3 direction, out vec3 p) {
	...
	for(int i = 0; i < STEPS; i++) {
		...
		float dist = opRep(p, vec3(2.0)); // Try with diverent vals for the vec3
...
void main() {
	...
	vec3 cameraPosition = vec3(cos(u_time) * 4.0, 0.0, sin(u_time) * 5.0);
