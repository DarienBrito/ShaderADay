...
float scene(vec3 p){
	//float b = udBox(p, vec3(0.5, 0.5, 1.0));
	float s = sphere(p + vec3(0.0,cos(u_time*0.5) * 0.25,0.0), 0.5);
	float t = sdTorus(p * rotate(sin(u_time * 0.5) * PI, 0), vec2(0.5, 0.2));
	return differenceSDF(s, t);
//	return t;
}
