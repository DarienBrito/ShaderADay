/*
Expanding on yesterday's code using 
a nice vector trick. Pretty expensive though 
--------------------
Shader-a-day
...
void main() {
  ...
  float dScale = 4.0; // Displacement scale for coordinate map
  ...
  for(int i = 0; i < 3; i++) {
    delta += 0.01; // Add a slight time difference to colors
    vec2 cp = vec2(cos(delta), sin(delta)) * 0.5 + 0.5; 
    float l = quadraticBezier(st.x, cp);
    float c = smoothstep(l, l + xRange, st.y); // Map the bezier curve along y
    color[i] = c; // Fill RGB values in the vector
  }
